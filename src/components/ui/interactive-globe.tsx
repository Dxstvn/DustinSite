...
export function InteractiveGlobe({
  ...
  // Fibonacci-distributed sphere dots
  ...
  const draw = useCallback(() => {
    ...
    for (const conn of connections) {
      ...
      // Skip if both endpoints face away
      if (z1 > radius * 0.3 && z2 > radius * 0.3) continue;

      const [sx1, sy1] = project(x1, y1, z1, cx, cy, fov);
      const [sx2, sy2] = project(x2, y2, z2, cx, cy, fov);

      // Elevated midpoint for quadratic arc
      const midX = (x1 + x2) / 2;
      const midY = (y1 + y2) / 2;
      const midZ = (z1 + z2) / 2;
  ...
  return (
    <canvas
      ref={canvasRef}
      role="img"
      aria-label="Interactive globe showing Jaspire service locations across the Americas and Europe"
      className={cn(
